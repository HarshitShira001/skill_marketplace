...
const TutorChatSessions = () => {
  ...
  useEffect(() => {
    ...
    // Listen for completion status updates
  ...
  const handleJoinCall = () => {
    ...
    navigate('/tutor/video-sessions', { state: { requestId: activeConv._id } });
  };

  const handleCompleteCall = async () => {
    if (!activeConv || releasing) return;
    setReleasing(true);
    try {
      await axios.put(`${SOCKET_URL}/api/payments/request-completion/${activeConv._id}`, {}, {
        headers: { Authorization: `Bearer ${token}` }
      });
      socketRef.current.emit('payment_status_change', { requestId: activeConv._id, status: 'completion_requested' });
      setActiveConv(prev => ({ ...prev, paymentStatus: 'completion_requested' }));
      setConversations(prev => prev.map(c => c._id === activeConv._id ? { ...c, paymentStatus: 'completion_requested' } : c));
      alert('Completion requested! Waiting for learner confirmation.');
    } catch (err) {
  ...
  const getOtherUser = (conv) => conv.learner;
              ...
              {loading ? (
                <div className="p-4 space-y-3">{[1,2,3].map(i => (
                  <div key={i} className="flex items-center gap-3 px-4 py-3 animate-pulse">
                    ...
                    <div className="flex-1 space-y-2"><div className="h-3 bg-gray-100 rounded-full w-24" /><div className="h-2 bg-gray-50 rounded-full w-16" /></div>
                  </div>
                ))}</div>
                ...
                return (
                  <button key={conv._id} onClick={() => { setActiveConv(conv); setMessages([]); }}
                    ...
                    <div className="w-12 h-12 rounded-2xl bg-indigo-600 flex items-center justify-center text-white font-black text-sm flex-shrink-0 shadow-lg shadow-indigo-100 uppercase overflow-hidden">
                      {other?.profilePhoto ? <img src={other.profilePhoto} className="w-full h-full object-cover" /> : (other?.name?.[0] || '?')}
                    </div>
            ...
            <div className="flex-1 flex flex-col bg-white">
              <div className="bg-white border-b border-gray-100 px-8 py-5 flex items-center justify-between">
                <div className="flex items-center gap-4">
                  <div className="w-12 h-12 rounded-2xl bg-indigo-600 flex items-center justify-center text-white font-black text-lg shadow-xl shadow-indigo-100 uppercase overflow-hidden">
                     {getOtherUser(activeConv)?.profilePhoto ? <img src={getOtherUser(activeConv).profilePhoto} className="w-full h-full object-cover" /> : (getOtherUser(activeConv)?.name?.[0] || '?')}
                  </div>
                  <div>
                    ...
                    <div className="flex items-center gap-2 mt-0.5">
                      ...
                      {(activeConv.paymentStatus === 'held' || activeConv.paymentStatus === 'released' || activeConv.paymentStatus === 'completion_requested') && 
                        <span className="text-[9px] font-black bg-emerald-50 text-emerald-600 px-2 py-0.5 rounded-full uppercase tracking-widest flex items-center gap-1.5">
                ...
                <div className="flex items-center gap-3">
                  <button onClick={handleJoinCall}
                    className="flex items-center gap-2.5 px-6 py-3 rounded-2xl font-black text-xs transition-all uppercase tracking-widest shadow-xl bg-indigo-600 text-white shadow-indigo-100 hover:scale-105 active:scale-95">
                ...
                {messages.map((msg, i) => {
                  ...
                  return (
                    <div key={msg._id || i} className={`flex ${isMe ? 'justify-end' : 'justify-start'} animate-in fade-in slide-in-from-bottom-2 duration-300`}>
                      {!isMe && (
                        <div className="w-8 h-8 rounded-xl bg-gray-100 flex items-center justify-center text-gray-400 font-bold text-xs mr-3 self-end shadow-sm uppercase overflow-hidden">
                          {getOtherUser(activeConv)?.profilePhoto ? <img src={getOtherUser(activeConv).profilePhoto} className="w-full h-full object-cover" /> : (getOtherUser(activeConv)?.name?.[0] || '?')}
                        </div>
                      )}
             ...
             <div className="flex-1 flex flex-col items-center justify-center bg-gray-50/40 gap-6">
                ...
                <div className="text-center">
                  ...
                  <p className="text-gray-400 text-sm font-medium mt-1">Pick a learner from the list to start chatting</p>
                </div>
             </div>
          )}
        </div>
...
export default TutorChatSessions;
